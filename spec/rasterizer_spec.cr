...
describe HallOfFame::RsvgRasterizer do
  it "produces a PNG from an SVG" do
    bytes = HallOfFame::RsvgRasterizer.new.rasterize(TINY_SVG, 2.0)
  ...
  it "fails with a helpful message when the binary is missing" do
    rasterizer = HallOfFame::RsvgRasterizer.new(binary: "rsvg-convert-nope")
    expect_raises(HallOfFame::RasterError, /rsvg-convert/) do
