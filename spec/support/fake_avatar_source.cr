...
class FakeAvatarSource < HallOfFame::AvatarSource
  getter fetch_count = 0

  def initialize(@missing : Array(String) = [] of String)
  end

  def url_for(user : HallOfFame::ResolvedUser, size : Int32) : String
    "fake://#{user.avatar_url || user.login}/#{size}"
  end

  def fetch(user : HallOfFame::ResolvedUser, size : Int32) : {Bytes, String}
    @fetch_count += 1
    if @missing.includes?(user.login)
      raise HallOfFame::AvatarError.new("avatar not found (404)", 404)
    end
